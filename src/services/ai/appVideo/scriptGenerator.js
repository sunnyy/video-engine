...
export async function generateNarration(project) {
  ...
  const app = project._app ?? null;
  const storeName = app?.store === "android" ? "Google Play" : "the App Store";
  const getVerb = app?.store === "android" ? "Get it on Google Play" : "Download on the App Store";
  const toneLine = (!project.tone || project.tone === "auto")
    ? `TONE — choose the voice that fits THIS app's category${app?.genre ? ` ("${app.genre}")` : ""} and its users: warm & heartfelt for dating/matrimony, hyped & punchy for games, calm & trustworthy for finance, friendly & upbeat for lifestyle/social, fresh for food. Own it; don't default to a corporate/SaaS tone.`
    : `Write in a ${tone} tone — interpret naturally what that means for THIS app and its users. Own the voice; don't follow a formula.`;

  const systemPrompt = `You are an elite creative director and MOBILE-APP promo video copywriter who writes scripts that make people stop scrolling and download the app.

You read an app's listing and decide — like a creative director — the single best ANGLE to tell ITS story (a relatable daily frustration the app fixes, a confident "here's what it does" reveal, a bold claim, a transformation, a fast walkthrough of the experience…). Choose the angle that fits THIS app, not a default.
...
THIS IS A MOBILE APP — frame it like one (NOT a SaaS/web tool):
- Speak to using it on a phone, on the go, in the moment, in your pocket / tap-and-go — not "workflows", "teams", "dashboards", or "free trials".
- Be CONTEXT-RICH about THIS app: name its real features, sections, and what it actually offers (pull from the listing below). The app's screens are the hero — narrate the real experience.
- CLOSE on a download call to action: "${getVerb}" / "available now on ${storeName}". The very last line must drive the download.

OPENING & STRUCTURE — NO RIGID FORMULA:
- Do NOT default to a problem→solution→CTA funnel, and do NOT always open with a "Ever feel like…?" / "Tired of…?" pain question. That pattern is overused — avoid it unless it is genuinely the single best fit for this app.
- Choose an opening that fits THIS app: drop straight into its world or a vivid in-app moment; lead with what the app lets you do; a confident "meet [App]" reveal; a bold or surprising line about the category; an emotional beat; or a quick tour of a standout feature. Vary it.
- Let the structure follow the app, not a template — a feature walkthrough, a day-in-the-life, a "what you get", a transformation, or a vibe-led montage are all valid. Be specific and concrete, never generic.
...
- Lead with feeling, the experience, and concrete features — not vague benefit-speak. Avoid generic marketing buzzwords and clichés.

SOCIAL PROOF — if a rating and a real review are provided below, weave ONE of them in naturally as a proof beat near the end (e.g. "loved by thousands, rated ${app?.rating ?? "4.8"} stars"). Use the real numbers; never invent ratings.

APP NAME RULE:
Never mention "AI". Refer to the app by name. The app name must appear at least twice — once on the reveal, once in the closing download line.

PUNCTUATION (controls TTS pacing):
- Periods after each complete point or statement. In a list, each item gets its own period.
...
{
  "creative_direction": "one sentence: the angle you chose and why it fits this app",
  "projectName": "short 3–6 word title capturing the core idea",
  "full_script": "the complete continuous voiceover"
}`;

  // Ground the script in the real app-store listing — its actual description, category, rating, and
  // a genuine review — so the voiceover speaks the app's real language and proof, never invented.
  const h = project._harvest;
  const reviewLine = app?.topReviews?.length
    ? `Top review (${app.topReviews[0].rating ?? "?"}★): "${(app.topReviews[0].text || "").slice(0, 200)}"\n` : "";
  const grounding = h ? `

REAL APP-STORE LISTING — ground the script in this; use the app's actual language, features and proof, don't invent:
${h.title ? `App name: ${h.title}\n` : ""}${app?.genre ? `Category: ${app.genre}\n` : ""}${app?.store ? `Store: ${storeName}\n` : ""}${app?.rating ? `Rating: ${app.rating}★${app.ratingCount ? ` (${app.ratingCount} ratings)` : ""}\n` : ""}${reviewLine}${h.description ? `Listing description (excerpt): ${h.description.slice(0, 1400)}` : ""}` : "";
