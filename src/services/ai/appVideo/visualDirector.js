...
export async function planVisualBeats({ full_script, wordTimestamps = [], audioDuration = 0, projectContext = {} }) {
  ...
  // App mode: the app's REAL screenshots are the hero — most beats must SHOW the app, not abstract
  // type scenes (which read as a SaaS web promo). This overrides the "lean heavily on html" default.
  const appNote = (isMobileApp && hasShots)
    ? `\n\nTHIS IS A MOBILE-APP PROMO — build a FAST, VARIED, app-ad sequence (NOT a slow slideshow, NOT a SaaS web deck). Mix THREE kinds of beats:
1) SHOW THE APP — "media_full" / "media_split" with media_source "product_shot" (the ${projectContext.screenshotCount} REAL screenshots). Use SEVERAL across the video to walk the key screens/features, ONE short caption each — but do NOT linger on one screen.
2) ATMOSPHERE & EMOTION — use "stock_image", "stock_video" (real people/places/moments — a couple, a celebration, a city street, hands on a phone) and "ai_image" (evocative concept shots) FREELY for variety and mood. Use SEVERAL of these to break up the screenshots and set feeling — they are NOT a last resort.
3) DESIGNED TYPE — a few "html" beats ONLY for the hook, a bold stat/rating, and the closing download CTA.
PACING — this is the #1 rule: MANY SHORT beats, 1.5–4s EACH, cut often; NEVER hold any frame longer than ~4s. A 35–40s video should be ~11–14 beats, NOT 5–6. If a span of speech is long, SPLIT it into separate beats with different visuals.
EVERY beat must FILL the frame — full-bleed media, or a fully-composed designed frame. No half-empty scenes, no long static holds.`
    : isMobileApp
    ? `\n\nTHIS IS A MOBILE-APP PROMO: keep the energy of a fast app ad (not a slow SaaS web deck). Mix designed type beats with plenty of stock_image / stock_video / ai_image for atmosphere, real people, and real-world context. MANY SHORT beats (1.5–4s each, ~11–14 for a 35–40s video) — cut often, never hold a frame >4s, and FILL every frame.`
    : (hasShots
...
TOTAL VOICEOVER DURATION: ~${(audioDuration || 0).toFixed(1)}s${appNote}
