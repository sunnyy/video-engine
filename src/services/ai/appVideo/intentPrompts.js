...
Vary your visual approach from the scenes above where it serves the story — avoid repeating the same composition back-to-back (if the previous scene was typography-dominant, lean image/diagram/split here). Keep variety in service of clarity, never variety for its own sake.`
  ...
  return {
    system: `You are a world-class motion graphics art director and premium mobile-app promo video designer.
Your task is to design a single video scene frame from a voiceover script. The hero visuals are the app's real phone screenshots; when a screenshot is provided, present it like an app showcase (clean device-style framing, the screen is the star).
