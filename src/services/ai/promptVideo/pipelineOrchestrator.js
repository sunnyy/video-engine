...
import { lintBeatGraph, repairBeatHTML } from "./beatLinter.js";
...
function revealKeyframes(layer, delay) {
  ...
  layer.keyframes = {
    ...
    opacity: [{ time: d, value: 0 }, { time: parseFloat((d + 0.25).toFixed(3)), value: 1 }],
    y:       [{ time: d, value: y + 34 }, { time: parseFloat((d + 0.25).toFixed(3)), value: y }],
  };
}
  ...
  for (const beat of beats) {
    ...
    if (beat.words && beat.words.length >= 3) {
      ...
      if (increasing) {
        ...
        console.log(`[prompt] kinetic sync: beat ${beat.beat_index} — ${matches.length} phrase reveals on speech`);
        synced = true;
      }
    }
    if (synced) continue;
    ...
    console.log(`[prompt] kinetic sync: beat ${beat.beat_index} — ${heroes.length} even-spread reveals (no word match)`);
  }
}
...
function splitLongBeats(beats) {
  ...
  for (const beat of beats) {
    ...
    console.log(`[prompt] split long beat ${beat.beat_index} (${dur.toFixed(1)}s) → "${partA}" + "${partB}"`);
  }
...
async function attachSfx(layers, beats) {
  try {
    ...
    if (attached) console.log(`[prompt] sfx: ${attached}x "${whoosh.key}"`);
  } catch (e) {
    console.warn("[prompt] sfx skipped:", e.message);
  }
}
...
export async function runPromptPipeline(params, onStep) {
  ...
  const step  = (msg) => { console.log(`[prompt] ${msg}`); onStep?.({ step: msg }); };
  ...
  if (plan?.research && plan?.film?.beats?.length) {
    ...
    film     = plan.film;
    console.log(`[prompt] using approved plan: ${film.beats.length} beats, style=${film.style?.id}`);
  } else {
    // ── Stage 0: Research ───────────────────────────────────────────────────
  ...
  try {
    ...
    console.warn("[prompt] TTS failed (non-fatal):", e.message);
  }
  ...
  if (beats.length > 0) {
    ...
    if (overshoot > 0) {
      ...
      if (overshoot > 1.5) console.warn(`[prompt] voiceover overshoot ${overshoot.toFixed(1)}s — capped last-beat pad at 1.5s`);
    }
  ...
  console.log(`[prompt] designing ${designedBeats.length}/${beats.length} beats (${beats.filter(b => isFullBleedBeat(b) && !hasOverlayContent(b)).length} clean shots)`);
  ...
  // ── Stage 5: Lint → repair → re-parse ────────────────────────────────────
  step(PROMPT_STATUS_STEPS[5]);
  const vocabText = [
    fullScript, research.topic,
    ...(research.facts ?? []), ...(research.artifacts ?? []),
    ...(research.entities ?? []).map(e => e.name),
    ...beats.flatMap(b => [b.content?.headline, b.content?.subtext, b.content?.attribution,
      ...(Array.isArray(b.content?.items) ? b.content.items.map(it => typeof it === "string" ? it : `${it?.label ?? ""} ${it?.value ?? ""}`) : [])]),
  ].filter(Boolean).join(" ");
  ...
  const beatGraphs = await Promise.all(beats.map(async (beat, i) => {
    ...
    let html  = beatHTMLs[i];
    let graph = parseSceneHTML(html || "", beat.beat_index, CANVAS);
    const opts = { canvasW: CANVAS.width, canvasH: CANVAS.height, isFullBleed: false, overlayMode, vocabText, paletteBg: palette.bg, language };

    const violations = lintBeatGraph(graph, opts);
    if (violations.length > 0 && html) {
      const repaired = await repairBeatHTML(html, violations, `beat ${beat.beat_index}`);
      if (repaired !== html) {
        const g2 = parseSceneHTML(repaired, beat.beat_index, CANVAS);
        // Accept only true repairs: violations must not grow, and neither may
        // the element count (a "repair" that balloons 6 → 28 elements is a redesign)
        if (g2.length >= 2 && g2.length <= graph.length + 4 && lintBeatGraph(g2, opts).length <= violations.length) {
          html = repaired; graph = g2; beatHTMLs[i] = repaired;
        } else {
          console.warn(`[prompt/lint] beat ${beat.beat_index}: repair rejected (${graph.length} → ${g2.length} elements)`);
        }
      }
    }
    return graph;
  ...
  try {
    ...
    if (tracks?.length) {
      ...
      console.log(`[prompt] music: "${track.title}" (${film.music_mood})`);
    }
  } catch (e) {
    console.warn("[prompt] music skipped:", e.message);
  }
  ...
  try {
    ...
    console.log(`[prompt] saved project: ${editorProjectId}`);
  } catch (e) {
    console.warn("[prompt] projects insert failed (non-fatal):", e.message);
  }
