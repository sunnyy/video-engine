...
export function buildBeatPrompt(beat, ctx) {
  ...
  // _overSpeaker: a Talking Head over-speaker overlay — use OVERLAY (transparent) mode with the
  // speaker (not a media asset) behind, so the person stays visible under the designed element.
  const isOverlay = beat._overSpeaker || (!!beat.asset?.src && (beat.asset.kind === "image" || beat.asset.kind === "video"));
  const behindDesc = beat._overSpeaker
    ? "the SPEAKER (a person talking to camera) is on screen"
    : `a cinematic ${beat.asset?.kind === "video" ? "clip" : "image"} plays full-bleed`;
...
NEVER print the beat's internal kind/role as visible text — words like "Hook", "Fact", "Stat", "Quote", "List", "CTA", "Title", "Reveal" are internal direction, never on screen. Render only the real CONTENT strings.`;
  ...
  if (isOverlay) {
    ...
    const system = `You design a PREMIUM KINETIC TEXT TREATMENT laid over one shot of a fast-cut video — on screen ${duration.toFixed(1)}s. Behind you, ${behindDesc} (the pipeline injects what's behind${beat._overSpeaker ? "" : " AND a legibility scrim"} — you build NEITHER). ${beat._overSpeaker ? "Keep your design COMPACT and OFF the centre/face — a small reinforcing element, not a full takeover. " : ""}Design with real CSS layout; a browser measures the result.
...
OUTPUT: only the HTML, from <!DOCTYPE html>. Root: html,body{width:${canvasW}px;height:${canvasH}px;margin:0;overflow:hidden;background:transparent}.`;
    ...
    const user = `BEAT ${beat.beat_index} — ${beat._overSpeaker ? "a compact element OVER the speaker" : `overlay on a shot of: ${beat.image_prompt ?? beat.shot_query ?? beat.visual_concept}`}
SPOKEN: "${beat.script_line ?? beat.spoken ?? ""}"
