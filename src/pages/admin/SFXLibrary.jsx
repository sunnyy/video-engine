...
const MOODS    = ["dramatic", "positive", "neutral", "tense", "playful", "peaceful", "transcendent", "chaotic", "negative", "kinetic", "appetizing", "celebratory", "triumphant", "energetic", "comedic", "silly", "urgent", "futuristic"];
const ENERGIES = ["low", "medium", "high"];

async function fetchTracks() {
  const { data, error } = await supabase.from("sfx_tracks").select("*").order("title");
...
function TrackRow({ track, onToggle, onDelete, toggling, deleting }) {
  ...
  return (
    <tr className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
      ...
      <td className="px-4 py-3 text-sm text-[#888] font-mono">{track.duration ? `${track.duration}s` : "—"}</td>
      <td className="px-4 py-3">
...
function UploadForm({ onUploaded }) {
  ...
  return (
    <div className="bg-[#111118] border border-white/[0.08] rounded-xl p-5 mb-6">
      <div className="text-base font-semibold text-white mb-4">Add Sound Effect</div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        ...
        <div>
          <div className="text-xs text-[#555] uppercase tracking-wider mb-1.5">Duration (seconds)</div>
    ...
    <AdminLayout>
      ...
      {loading ? (
        ...
        <div className="text-[#444] text-base py-16 text-center">
          No sound effects yet. Upload one above.
        </div>
      ) : (
        <div className="bg-[#111118] border border-white/[0.06] rounded-xl overflow-hidden">
          <div className="px-4 py-2.5 text-xs text-[#555] border-b border-white/[0.06]">
            {tracks.length} sound effect{tracks.length !== 1 ? "s" : ""}
          </div>
