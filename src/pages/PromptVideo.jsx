...
function VideoCard({ project, onDelete }) {
  ...
  const navClick = e => { if (!e.ctrlKey && !e.metaKey) { e.preventDefault(); navigate(editorHref, { state: { from: "/prompt-video" } }); } };
...
function GeneratorForm() {
  ...
  async function handleProduce() {
    ...
    try {
      ...
      invalidateProjectCaches("prompt_video", "all");
      navigate(`/video-editor/${result.projectId}`, { state: { from: "/prompt-video" } });
    } catch (err) {
