...
/**
 * BrandKit — manage the user's brand kit (logo, channel name, CTA, website, colors).
 * Applied to generated videos as a post-process (shared/brandKit.js) — logo bug + CTA.
 * Lives at /brand-kit (linked from AppLayout). Not part of Settings.
 */
    ...
    <AppLayout>
      <div style={{ flex: 1, overflowY: "auto", background: T.bg }}>
        <div style={{ maxWidth: 620, margin: "0 auto", padding: "44px 24px 80px" }}>
          ...
          {loading ? (
            <div style={{ color: T.faint, fontSize: 14 }}>Loading…</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div>
                ...
                <input style={field} value={form.channel_name} onChange={(e) => set("channel_name", e.target.value)} placeholder="e.g. Vidquence" />
              </div>

              <div>
                <span style={label}>Call to action (closing line)</span>
                <input style={field} value={form.cta_text} onChange={(e) => set("cta_text", e.target.value)} placeholder="e.g. Follow for more" />
              </div>

              <div>
                <span style={label}>Website (optional)</span>
                <input style={field} value={form.website} onChange={(e) => set("website", e.target.value)} placeholder="https://…" />
              </div>
