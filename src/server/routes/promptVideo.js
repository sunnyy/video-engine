...
router.post("/plan", requireAuth, async (req, res) => {
  try {
    ...
    console.error("[prompt-video/plan]", e.message);
...
router.post("/generate", requireAuth, async (req, res) => {
  ...
  try {
    const deduction = await deductCredits(userId, PROMPT_VIDEO_CREDITS, "prompt_video", "Prompt to Video generation", null);
    ...
    if (creditAmount > 0) {
      addCredits(userId, creditAmount, "refund", "ai_failure_refund", "Refund: Prompt video failed").catch(() => {});
    }
    console.error("[prompt-video/generate]", err);
