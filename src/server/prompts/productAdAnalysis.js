  ...
  "shots": [
    {
      ...
      "image_generation_prompt": "For clothing/wearable Shot 1: open with the model_description verbatim, then 'wearing the exact garment from the reference photo', then describe the scene. For clothing/wearable Shot 2+: open with 'The same Indian woman from shot 1, [model_description], wearing the exact same garment', then describe the scene. For non_worn: start with 'Use the uploaded photo as the product reference. Keep the same branding, label, colors, and identity.' then describe ONLY the scene. All prompts end with: hyper-realistic, photorealistic, 9:16 vertical portrait, no text overlays.",
...
IMPORTANT for image_generation_prompt (clothing/wearable): the no-redesign rule must be embedded — add "Do not alter, redesign, or reimagine the garment or product in any way. Preserve every design detail exactly: colors, patterns, prints, embroidery, cut, collar, sleeves, hem, buttons, and any graphic elements." before the scene description.
