/**
 * decorativeShapeRegistry.js
 * src/core/registries/decorativeShapeRegistry.js
 *
...
const decorativeShapeRegistry = {
  ...
  // ── Gradient-ready gradient shapes ───────────────────────
  gradient_bar: {
...
export default decorativeShapeRegistry;

// ─────────────────────────────────────────────────────────────────────────────
// Picker options
// ─────────────────────────────────────────────────────────────────────────────
export const DECORATIVE_SHAPE_OPTIONS = Object.entries(decorativeShapeRegistry).map(([id, entry]) => ({
  id,
  label:    entry.label,
  icon:     entry.icon,
  group:    entry.group,
...
export function getClipPathCSS(shapeId) {
  return decorativeShapeRegistry[shapeId]?.clipPath || null;
}
...
export function renderDecorativeSVG(shapeId, style = {}, instanceId = "x") {
  const reg = decorativeShapeRegistry[shapeId];
  if (!reg) return null;
  ...
  // Build fill/stroke refs (gradient or solid)
